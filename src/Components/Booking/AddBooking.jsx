...
const AddBooking = () => {
  ...
  return (
    ...
    >
                      ...
                      >
                        ...
                        <TableCell>
                          <Input
                            type="number"
                            min="1"
                            max={roadSign ? roadSign.faces_number - roadSign.total_faces_on_date : 1} // استخدم roadSign للوصول إلى faces_number و total_faces_on_date
      ...
      <Dialog open={contractDialog} onOpenChange={setContractDialog}>
        <DialogContent
          dir="rtl"
          className="w-full max-w-[90vw] max-w-md p-6"
        >
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-gray-900 dark:text-gray-100 text-right">
              {parseInt(formik.values.type) === 1
  ...
  {/* استبدال Textarea بمكون ReactQuill */}
  <ReactQuill
    theme="snow" // يمكنك اختيار ثيم آخر مثل "bubble"
    value={notes}
    onChange={setNotes}
    directions="rtl"
    className="w-full text-right dark:text-gray-200"
    placeholder="أدخل الملاحظات هنا..."
    // Add custom styling here to match shadcn's theme
    // We'll apply these styles using a global CSS file or a <style> tag if preferred
  />
</div>
