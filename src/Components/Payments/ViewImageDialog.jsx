// src/utils/Dialogs/ViewImageDialog/ViewImageDialog.jsx
import React from 'react';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { Download, Maximize } from "lucide-react";
import { toast } from "sonner";
import { showToast } from '../../utils/Notifictions/showToast';

export const ViewImageDialog = ({ show, handleClose, imageUrl, paymentNumber }) => {
  const handleDownload = () => {
    if (imageUrl) {
      const link = document.createElement('a');
      link.href = imageUrl;
      link.download = `payment_${paymentNumber || 'image'}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      showToast("success", "تم تنزيل الصورة بنجاح.");
    } else {
      showToast("error", "لا توجد صورة لتنزيلها.");
    }
  };

  const handleMaximize = () => {
    if (imageUrl) {
      window.open(imageUrl, '_blank');
    } else {
      showToast("error", "لا توجد صورة لتكبيرها.");
    }
  };

  return (
    <Dialog open={show} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[425px] overflow-auto dialog-content max-h-[600px] md:max-w-xl lg:max-w-3xl">
        <DialogHeader>
          <DialogTitle className="text-right">عرض صورة الدفعة</DialogTitle>
          <DialogDescription className="text-right">
            صورة الدفعة رقم: {paymentNumber || 'غير متوفر'}
          </DialogDescription>
        </DialogHeader>
        <div className="flex justify-center items-center py-4">
          {imageUrl ? (
            <img
              src={imageUrl}
              alt={`صورة الدفعة رقم ${paymentNumber}`}
              className="max-w-[50%] h-auto rounded-md object-contain"
              onError={(e) => {
                e.target.onerror = null;
                e.target.src = "data:image/png;base64,";
                toast.error("تعذر تحميل الصورة.");
              }}
            />
          ) : (
            <div className="w-full h-48 flex items-center justify-center bg-gray-100 text-gray-500 rounded-md border border-dashed">
              لا توجد صورة متوفرة لهذه الدفعة.
            </div>
          )}
        </div>
        <div className="flex justify-end gap-2 mt-4">
          <Button onClick={handleDownload} disabled={!imageUrl} variant="outline">
            <Download className="mr-2 h-4 w-4" /> تنزيل
          </Button>
          <Button onClick={handleMaximize} disabled={!imageUrl} variant="outline">
            <Maximize className="mr-2 h-4 w-4" /> تكبير
          </Button>
        </div>
      </DialogContent>
    </Dialog>
  );
};